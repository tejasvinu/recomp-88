...
import type { WorkoutProgress, SessionHistory, WorkoutSession } from "./types";
...
export default function App() {
  ...
  // ─── BW quick-fill (toggle all weights for an exercise) ───────────────────
  ...
  const handleExport = () => {
    const blob = new Blob(
      [JSON.stringify({ progress, sessions, exportedAt: new Date().toISOString() }, null, 2)],
  ...
  const timerPercent = restTimerDuration > 0 && restTimer !== null
    ? (restTimer / restTimerDuration) * 100
    : 0;
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-neutral-200 font-sans selection:bg-lime-400/30 overflow-hidden relative">
          ...
          {activeTab === "workout" && (
            <div className="flex flex-col gap-4">
              {activeDay.exercises.map((exercise, exIdx) => {
                const isBwActive = exercise.sets.every(
                  (set) => (progress[activeDay.id]?.[exercise.id]?.[set.id]?.loggedWeight ?? "") === "BW"
                ...
                return (
                  <div key={exercise.id} className="bg-neutral-900/50 border border-white/6 p-4 rounded-2xl flex flex-col gap-3 shadow-lg">
                    ...
                    {/* Column Headers */}
                    {exercise.type !== "other" && (
                      <div className="grid grid-cols-[24px_1fr_1fr_44px] gap-2 px-1 text-[9px] font-bold text-neutral-600 uppercase tracking-[0.15em] text-center">
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            exercise.type !== "other" ? "grid-cols-[24px_1fr_1fr_44px]" : "grid-cols-[1fr_44px]",
                            isCompleted
                        ...
                        >
                          {exercise.type !== "other" && (
                            <div className={cn("text-[11px] font-mono font-bold text-center tabular-nums", isCompleted ? "text-lime-400" : "text-neutral-600")}>
                              {setIdx + 1}
                            </div>
                          )}
                          ...
                          {exercise.type !== "other" && (
                            isBwSet ? (
                              ...
                              <input
                                ...
                                className={cn(
                                  "w-full bg-white/6 border border-white/8 text-center font-mono font-medium outline-none rounded-lg py-2 px-2 text-sm transition-all text-white placeholder-neutral-700 focus:ring-1 focus:ring-lime-400/40 focus:bg-white/9 focus:border-lime-400/30",
              ...
              <div className="mt-6 mb-4">
                <button
                  ...
                  className="w-full bg-white/4 hover:bg-white/7 active:scale-[0.98] border border-white/9 flex items-center justify-center gap-2.5 py-4 rounded-2xl text-white font-bold text-[12px] tracking-[0.18em] uppercase transition-all"
                >
        ...
        >
          ...
          <div className={cn(
            ...
            <div className="text-center">
              <p className="text-3xl font-black text-white uppercase tracking-[0.1em]">Day Done</p>
              <p className="text-[13px] text-lime-400 font-bold uppercase tracking-widest mt-1">{activeDay.name}</p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <button
              ...
              className="w-8 h-8 bg-white/5 hover:bg-white/[0.1] rounded-full flex items-center justify-center text-neutral-500 hover:text-white transition-colors border border-white/7"
            >
              <X size={14} />
            </button>
          </div>
                ...
                return (
                  ...
                  >
                    <div className={cn(
                      ...
                      isActive ? "bg-lime-400/12 border border-lime-400/20" : "border border-transparent"
                    )}>
      ...
      {/* ═══ Settings Modal ═══ */}
      {showSettings && (
        <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center" onClick={() => setShowSettings(false)}>
          ...
          >
            ...
            <div className="p-5 flex flex-col gap-5 overflow-y-auto max-h-[70vh] overscroll-contain">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {[
                    { label: "Strength", desc: "After strength sets", value: strengthRestDuration, set: setStrengthRestDuration },
                    { label: "Hypertrophy", desc: "After hypertrophy sets", value: hypertrophyRestDuration, set: setHypertrophyRestDuration },
                  ].map(({ label, desc, value, set }) => (
                    <div key={label} className="flex items-center justify-between bg-white/4 border border-white/7 rounded-xl px-4 py-3">
                      <div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="text-[13px] font-bold text-white group-hover:text-lime-400 transition-colors">Export JSON</p>
                      <p className="text-[10px] text-neutral-500 font-medium mt-0.5">Download all progress &amp; session history</p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-[10px] text-neutral-500 font-medium mt-0.5">Restore from a previous export</p>
                    </div>
                  </button>
